...
**Hallazgo** reads `hallazgos_inventario.json` (`arfershop.radar.hallazgos_inventario.v1`) and renders the 5-decision catalog: Recomprar · Precio bueno · Esperar · No · Manual. A 404 while Codex is still emitting the JSON does not break the other tabs. Deep-link: `/radar/#hallazgo`. **Exportar CSV** / **Copiar CSV** write `radar-hallazgo.csv` (all items, or the active 5-state filter). Import CSV for Hallazgo is a later PR — do not confuse with Más oportunidades.
