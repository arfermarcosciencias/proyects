...
**Hook RESURGIDO (Codex #14):** al importar con éxito se llama `refreshResurgidos()` (status `RESURGIDO`, `resurrected_at`, `resurrect_reason=gate_ok_post_import`, `prev_state`, `cured_pause_reason`, root `resurgidos[]`). El resultado del import se guarda en `radar-hoy:csv_import_result:v1` (`hook: refreshResurgidos`) y se espeja al Mac bridge `/api/actions` si está vivo (`type: csv_import`). No hay UI de mover Ya→Hoy (PR aparte).
